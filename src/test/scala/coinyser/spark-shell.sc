...
def filterTxs(txs: Dataset[Transaction]): Dataset[Transaction] = txs.filter("timestamp >= '2018-08-16 08:00:00'")
...
val grp2 = ds2.groupBy(window($"timestamp", "1 minute").as("w2")).agg(count($"tid").as("cnt2"))

val ds = filterTxs(spark.read.parquet("/home/mikael/projects/scala-fundamentals/bitcoin-analyser/data/transactions2/currency_pair=btcusd").as[Transaction])
val grp1 = ds.groupBy(window($"timestamp", "1 minute").as("w1")).agg(count($"tid").as("cnt1"))
